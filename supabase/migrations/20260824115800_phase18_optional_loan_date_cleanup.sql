drop function if exists public.abhi_apply_legacy_due_dates(uuid,jsonb,integer,text);
...
begin
    ...
    if v_update_count < 1 then raise exception 'At least one EMI update is required'; end if;
    ...
    if p_loan_year is not null and (p_loan_year < 2000 or p_loan_year > 2200) then
        raise exception 'Invalid loan year';
    end if;
    if p_loan_date is not null and (extract(year from p_loan_date)::integer < 2000 or extract(year from p_loan_date)::integer > 2200) then
        raise exception 'Invalid loan date';
    end if;
    if p_loan_year is not null and p_loan_date is not null and extract(year from p_loan_date)::integer <> p_loan_year then
        raise exception 'Loan year must match the exact loan date year';
    end if;

    select count(*) into v_missing_count
    from public.emis
    where loan_id = p_loan_id and (due_year is null or due_date is null);

    if v_missing_count = 0 then raise exception 'This loan has no legacy EMI date gaps'; end if;
    if v_update_count <> v_missing_count then
    ...
    -- Validate every row before creating the snapshot or changing data.
    ...
    loop
        ...
        v_month := case upper(trim(coalesce(v_emi.due_month,'')))
            ...
            when 'SEP' then 9 when 'OCT' then 10 when 'NOV' then 11 when 'DEC' then 12 else null end;
        if v_month is null or v_emi.due_day is null or v_emi.due_day < 1 or v_emi.due_day > 31 then
            raise exception 'Legacy EMI has invalid day/month and cannot be auto-fixed';
        end if;
        begin
            v_due_date := make_date(v_year, v_month, v_emi.due_day);
        exception when datetime_field_overflow then
            raise exception 'Invalid calendar date for EMI #% (% % %)', v_emi.installment_number, v_emi.due_day, v_emi.due_month, v_year;
        end;
    end loop;

    v_label := 'Before Phase 18 date cleanup - ' || coalesce(nullif(trim(v_loan.loan_code),''), p_loan_id::text);
    v_snapshot_id := public.abhi_create_backup_snapshot(v_label, 'pre-phase18-data-quality');

    if p_loan_year is not null and v_loan.loan_year is null then
        update public.loans set loan_year = p_loan_year where id = p_loan_id;
    elsif p_loan_year is not null and v_loan.loan_year is not null and v_loan.loan_year <> p_loan_year then
        raise exception 'Existing loan year cannot be overwritten from Data Quality Center';
    end if;
    if p_loan_date is not null and v_loan.loan_date is null then
        update public.loans set loan_date = p_loan_date where id = p_loan_id;
    elsif p_loan_date is not null and v_loan.loan_date is not null and v_loan.loan_date <> p_loan_date then
        raise exception 'Existing loan date cannot be overwritten from Data Quality Center';
    end if;
    ...
    values (
        'LEGACY_DATE_CLEANUP', 'loans', p_loan_id::text,
        'Legacy EMI dates reviewed: ' || v_updated || ' row(s)' ||
    ...
    return jsonb_build_object(
        'success', true,
        'loan_id', p_loan_id,
        'updated_emis', v_updated,
        'loan_year', coalesce(p_loan_year, v_loan.loan_year),
        'loan_date', coalesce(p_loan_date, v_loan.loan_date),
