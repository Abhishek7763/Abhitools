-- Phase 11: include settlement state in Data Safety backups.
...
begin
    v_payload := jsonb_build_object(
        'format','abhitools-snapshot','version',4,'created_at',now(),
    ...
    v_summary := jsonb_build_object(
        'borrowers',(select count(*) from public.borrowers),'loans',(select count(*) from public.loans),'emis',(select count(*) from public.emis),
        'loan_settlements',(select count(*) from public.loan_settlements where reopened_at is null),
        'emi_payments',(select count(*) from public.emi_payments where reversed_at is null),'documents',(select count(*) from public.documents)
    );
...
begin
    ...
    from jsonb_to_recordset(coalesce(v_payload->'loan_settlements','[]'::jsonb)) as x(id uuid,loan_id uuid,settlement_date date,scheduled_remaining_before integer,final_payment_amount integer,waived_amount integer,method text,notes text,created_at timestamptz,reopened_at timestamptz,reopen_note text);
    ...
    from jsonb_to_recordset(coalesce(v_payload->'emi_payments','[]'::jsonb)) as x(id uuid,emi_id uuid,amount integer,payment_date date,method text,notes text,source text,settlement_id uuid,created_at timestamptz,updated_at timestamptz,reversed_at timestamptz);
